...
EditLocalParameterDialog::EditLocalParameterDialog(QWidget *parent = nullptr)
    : QDialog(parent) {
  m_uiForm.setupUi(this);
  m_uiForm.logValueSelector->setCheckboxShown(true);
  m_uiForm.tableWidget->setItemDelegateForColumn(
      VALUE_COLUMN, new LocalParameterItemDelegate(this));
...
void EditLocalParameterDialog::subscribe(
    EditLocalParameterDialogSubscriber *subscriber) {
...
void EditLocalParameterDialog::logValueChanged(int index) {
  const auto &name = m_uiForm.logValueSelector->getLog();
  const auto &mode = m_uiForm.logValueSelector->getFunctionText();
  m_subscriber->setValueToLog(name, mode, index);
}

void EditLocalParameterDialog::logValueChanged() {
  const auto &name = m_uiForm.logValueSelector->getLog();
  const auto &mode = m_uiForm.logValueSelector->getFunctionText();
  m_subscriber->setValuesToLog(name, mode);
}

QString EditLocalParameterDialog::getValueAt(int row) const {
  m_uiForm.tableWidget->item(row, VALUE_COLUMN)->text();
}
...
void EditLocalParameterDialog::addRowToTable(QString const &datasetName,
  ...
  m_uiForm.tableWidget->setVerticalItemHeader(
