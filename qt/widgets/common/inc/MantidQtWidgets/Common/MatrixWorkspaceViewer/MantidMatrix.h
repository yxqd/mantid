...
*/
class EXPORT_OPT_MANTIDQT_COMMON MantidMatrix : public QFrame, MantidQt::API::WorkspaceObserver {
  Q_OBJECT

    MantidMatrix(const MantidMatrix&) = delete;

public:
  MantidMatrix(QWidget *parent=nullptr);
  MantidMatrix(Mantid::API::MatrixWorkspace_const_sptr ws, QWidget *parent,
               const QString &label = QString(), const QString &name = QString(),
