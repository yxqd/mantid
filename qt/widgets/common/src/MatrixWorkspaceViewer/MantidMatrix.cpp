#include "MantidQtWidgets/Common/MatrixWorkspaceViewer/MantidMatrix.h"
//#include "../ApplicationWindow.h"
//#include "../Graph3D.h"
//#include "../Spectrogram.h"
#include "MantidKernel/Logger.h"
#include "MantidQtWidgets/Common/MatrixWorkspaceViewer/MantidMatrixDialog.h"
//#include "MantidQtWidgets/Common/MatrixWorkspaceViewer/MantidMatrixFunction.h"
#include "MantidQtWidgets/Common/MatrixWorkspaceViewer/MantidMatrixModel.h"
//#include "MantidUI.h"
...
MantidMatrix::MantidMatrix(QWidget *parent): QFrame(parent), WorkspaceObserver() {}
...
MantidMatrix::MantidMatrix(Mantid::API::MatrixWorkspace_const_sptr ws,
    ...
    : QFrame(parent), WorkspaceObserver(), m_label(label), m_name(name),
  ...
  while (it != m_extensions.cend()) {
    ...
    case MantidMatrixModel::DX:
      if (ws->hasDx(0)) {
        ...
        connectTableView(extension.tableView.get(), extension.model);
        auto format = m_extensionRequest.getFormat(
            type, m_extensions, 10/*MantidPreferences::MantidMatrixNumberFormatDx()*/);
        auto precision = m_extensionRequest.getPrecision(
            type, m_extensions, 10 /*MantidPreferences::MantidMatrixNumberPrecisionDx()*/);
        setNumberFormat(modelTypeToInt(type), format, precision);
        ++it;
      } else {
        closeDependants();
