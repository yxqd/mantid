#include "MantidWorkflowAlgorithms/ChopAndReducePowder.h"
...
DECLARE_ALGORITHM(ChopAndReducePowder)

//----------------------------------------------------------------------------------------------
/** Initialisation method. Declares properties to be used in algorithm.
 */
void ChopAndReducePowder::init() {
  ...
  declareProperty(
      Kernel::make_unique<API::WorkspaceProperty<API::MatrixWorkspace>>(
          "SplitterWorkspace", "", Direction::Output),
  ...
  declareProperty("TMin", EMPTY_DBL(),
                  "Minimum for TOF or dspace axis. Defaults to 0.");
  ...
  //                  "Optional: Spectrum Nos (note that it is not detector ID
  //                  or "
  //                  "workspace indices).");
  //  declareProperty(make_unique<ArrayProperty<double>>("L2"),
  //                  "Optional: Secondary flight (L2) paths for each
  //                  detector");
  ...
  //                  "If offset is equal to 0, then the low resolution will
  //                  have "
  ...
  declareProperty("NumberOfThreads", 1, "Number of threads");
}
 ...
 */
double
ChopAndReducePowder::getVecPropertyFromPmOrSelf(const std::string &name,
...
ChopAndReducePowder::ChopWorkspace(const std::__cxx11::string &ws_group_name) {

  // generate filters
  API::IAlgorithm_sptr alg = createChildAlgorithm("GenerateEventsFilter");
  alg->initialize();
  alg->setProperty("InputWorkspace", m_inputW);

  MatrixWorkspace_sptr splitter_ws;
  ITableWorkspace_sptr info_ws;
  //  alg->setProperty("OutputWorkspace", getProperty("SplitterWorkspace"));
  //  alg->setProperty("InformationWorkspace", info_ws);
  alg->setProperty("FastLog", true);
  // TODO FIXME Shall input a vector
  alg->setPropertyValue("TimeInterval", "300.");
  alg->executeAsChildAlg();

  g_log.notice("1");
  info_ws = alg->getProperty("InformationWorkspace");
  g_log.notice() << "number of rows in info workspace: " << info_ws->rowCount()
                 << "\n";
  g_log.notice("1.5");
  API::Workspace_sptr out_ws = alg->getProperty("OutputWorkspace");
  splitter_ws = boost::dynamic_pointer_cast<API::MatrixWorkspace>(out_ws);
  g_log.notice("1.7");
  //  setProperty("SplitterWorkspace", splitter_ws);
  //  setProperty("InformationWorkspace", info_ws);
  g_log.notice("2");

  // chop
  API::IAlgorithm_sptr chopper = createChildAlgorithm("FilterEvents");
  chopper->initialize();
  chopper->setProperty("InputWorkspace", m_inputW);
  chopper->setProperty("SplitterWorkspace", splitter_ws);
  chopper->setProperty("InformationWorkspace", info_ws);
  chopper->setProperty("OutputWorkspaceBaseName", "FiveMinutes");
  chopper->setProperty("FilterByPulseTime", true);
  chopper->setProperty("GroupWorkspaces", false);
  chopper->setProperty("SplitSampleLogs", false);
  chopper->setProperty("OutputWorkspaceIndexedFrom1", true);
  chopper->executeAsChildAlg();

  std::vector<std::string> output_name_vector =
      chopper->getProperty("OutputWorkspaceNames");

  // create empty workspace grou
  API::WorkspaceGroup_sptr group; // = boost::make_shared<WorkspaceGroup>();

  API::IAlgorithm_sptr grouper = createChildAlgorithm("GroupWorkspaces");
  grouper->initialize();
  grouper->setProperty("InputWorkspaces", output_name_vector);
  //  grouper->setProperty("OutputWorkspace", group);
  grouper->executeAsChildAlg();

  group = grouper->getProperty("OutputWorkspace");
  setProperty("OutputWorkspace", group);
  setProperty("SplitterWorkspace", splitter_ws);
  setProperty("InformationWorkspace", info_ws);

  return output_name_vector;
}
...
ChopAndReducePowder::RebinWorkspace(const std::string &ws_name) {
  ...
  rebin3Alg->setPropertyValue("InputWorkspace", ws_name);
  // rebin3Alg->setProperty("OutputWorkspace", matrixws);
  rebin3Alg->setPropertyValue("Params", "5000, -0.001, 50000.");
  // rebin3Alg->setProperty("Params", m_params);
  rebin3Alg->executeAsChildAlg();
  API::MatrixWorkspace_sptr matrixws =
      rebin3Alg->getProperty("OutputWorkspace");
  return matrixws;
}

//---
API::MatrixWorkspace_sptr ChopAndReducePowder::CompressEventsInWorkspace(
    API::MatrixWorkspace_sptr matrix_ws) {
  EventWorkspace_sptr event_ws =
      boost::dynamic_pointer_cast<EventWorkspace>(matrix_ws);
  ...
  compressAlg->setProperty("InputWorkspace", matrix_ws);
  ...
  EventWorkspace_sptr out_event_ws =
      compressAlg->getProperty("OutputWorkspace");
  API::MatrixWorkspace_sptr out_matrix_ws =
      boost::dynamic_pointer_cast<MatrixWorkspace>(out_event_ws);

  return out_matrix_ws;
}

//--------------------
API::MatrixWorkspace_sptr
ChopAndReducePowder::AlignDetectors(API::MatrixWorkspace_sptr matrix_ws) {
  EventWorkspace_sptr event_ws =
      boost::dynamic_pointer_cast<EventWorkspace>(matrix_ws);
  //  g_log.information() << "running AlignDetectors started at "
  //                      << Types::Core::DateAndTime::getCurrentTime() << "\n";
  API::IAlgorithm_sptr alignAlg = createChildAlgorithm("AlignDetectors");
  alignAlg->setProperty("InputWorkspace", event_ws);
  ...
  API::MatrixWorkspace_sptr out_matrix_ws =
      alignAlg->getProperty("OutputWorkspace");
  //   =
  //      boost::dynamic_pointer_cast<MatrixWorkspace>(out_event_ws);
...
//--------------------
void ChopAndReducePowder::ReduceChoppedPowderData(
    const std::vector<std::string> &ws_name_vector) {

  size_t num_ws = ws_name_vector.size();
  size_t num_threads = number_of_threads_;
  size_t num_ws_thread = num_ws / num_threads;

  std::vector<Types::Core::DateAndTime> reduce_start_vector(num_threads);
  std::vector<Types::Core::DateAndTime> reduce_end_vector(num_threads);
  ...
  for (size_t ithread = 0; ithread < num_threads; ++ithread) {
    ...
    for (size_t i = 0; i < num_ws_thread; ++i) {
      // reduce data
      size_t chop_index = i * num_threads + ithread;
      std::string ws_name = ws_name_vector[chop_index];
      API::MatrixWorkspace_sptr matrix_ws = RebinWorkspace(ws_name);
      matrix_ws = CompressEventsInWorkspace(matrix_ws);
      matrix_ws = AlignDetectors(matrix_ws);
    }

    Types::Core::DateAndTime end_time =
        Types::Core::DateAndTime::getCurrentTime();

    //  PARALLEL_CRITICAL(ChopAndReducePowder_Reduce) {
    //   ;
    //          g_log.notice() << "Done with workspace " << chop_index  << ": "
    //          << matrix_ws->getName()
    //                              << "\n";

    //   }

    reduce_start_vector[ithread] = start_time;
    reduce_end_vector[ithread] = end_time;
  ...
  for (size_t i = 0; i < num_threads; ++i) {
    Types::Core::DateAndTime start_time = reduce_start_vector[i];
    reportss << start_time.toISO8601String() << "  -->  "
             << reduce_end_vector[i].toISO8601String() << ": dT = "
             << static_cast<double>(reduce_end_vector[i].totalNanoseconds() -
                                    start_time.totalNanoseconds()) *
                    1.E-9
             << "\n";
  }
  g_log.notice(reportss.str());
...
void ChopAndReducePowder::exec() {
  ...
  number_of_threads_ = static_cast<size_t>(numthreads);

  // split input workspaces
  Types::Core::DateAndTime chop_start_time =
      Types::Core::DateAndTime::getCurrentTime();
  std::string ws_group_name = getPropertyValue("OutputWorkspace");
  std::vector<std::string> ws_name_vector = ChopWorkspace(ws_group_name);

  // load calibration
  Types::Core::DateAndTime load_cal_start_time =
      Types::Core::DateAndTime::getCurrentTime();
  ...
  // SNS powder reduction
  Types::Core::DateAndTime reduce_start_time =
      Types::Core::DateAndTime::getCurrentTime();
  ReduceChoppedPowderData(ws_name_vector);
  Types::Core::DateAndTime end_time =
      Types::Core::DateAndTime::getCurrentTime();

  int64_t chop_time = load_cal_start_time.totalNanoseconds() -
                      chop_start_time.totalNanoseconds();
  int64_t load_cal_time = reduce_start_time.totalNanoseconds() -
                          load_cal_start_time.totalNanoseconds();
  int64_t reduce_time =
      end_time.totalNanoseconds() - reduce_start_time.totalNanoseconds();
  g_log.notice() << "Chop time     = " << static_cast<double>(chop_time) * 1.E-9
                 << "\n"
                 << "Load cal time = "
                 << static_cast<double>(load_cal_time) * 1.E-9 << "\n"
                 << "Reduce time   = "
                 << static_cast<double>(reduce_time) * 1.E-9 << "\n";

  int numThreads = static_cast<int>(PARALLEL_GET_MAX_THREADS);
  g_log.notice() << "maximum number of threas = " << numThreads << "\n";
  ...
  //          "Input workspace is not EventWorkspace.  It is not supported
  //          now.");
  //    } else {
  ...
  //      createChildAlgorithm("CompressEvents");
  ...
  //      m_outputEW = boost::dynamic_pointer_cast<EventWorkspace>(m_outputW);
  //    } else {
  //      g_log.information("skipping RemovePromptPulse on empty
  //      EventWorkspace");
  //    }
  //  }
  //  m_progress->report();
  ...
  //  if (LRef > 0.) {
  //    g_log.information() << "running UnwrapSNS(LRef=" << LRef << ",Tmin=" <<
  //    tmin
  ...
  //    API::IAlgorithm_sptr removeAlg =
  //    createChildAlgorithm("RemoveLowResTOF");
  //    removeAlg->setProperty("InputWorkspace", m_outputW);
  //    removeAlg->setProperty("OutputWorkspace", m_outputW);
  ...
  //      if (m_processLowResTOF) {
  //        m_lowResW = editInstrument(m_lowResW, tthsSplit.low,
  //        specidsSplit.low,
...
/** Call edit instrument geometry
  */
API::MatrixWorkspace_sptr ChopAndReducePowder::editInstrument(
...
/** Call diffraction focus to a matrix workspace.
  */
API::MatrixWorkspace_sptr
ChopAndReducePowder::diffractionFocus(API::MatrixWorkspace_sptr ws) {
  if (!m_groupWS) {
...
/** Convert units
  */
API::MatrixWorkspace_sptr
ChopAndReducePowder::convertUnits(API::MatrixWorkspace_sptr matrixws,
                                  std::string target) {
...
*/
API::MatrixWorkspace_sptr
ChopAndReducePowder::rebin(API::MatrixWorkspace_sptr matrixws) {
  if (m_resampleX != 0) {
    // ResampleX
...
ChopAndReducePowder::conjoinWorkspaces(API::MatrixWorkspace_sptr ws1,
...
void ChopAndReducePowder::convertOffsetsToCal(
 ...
 */
void ChopAndReducePowder::loadCalFile(const std::string &calFilename,
 ...
 */
void ChopAndReducePowder::doSortEvents(Mantid::API::Workspace_sptr ws) {
