...
class EnggCalibrate(PythonAlgorithm):
    ...
    def validateInputs(self):
        ...
        if not self.getPropertyValue(
                "ExpectedPeaksFromFile") and not self.getPropertyValue(
                    'ExpectedPeaks'):
    ...
    def PyExec(self):
        ...
        difa, difc, zero, fitted_peaks = self._fit_params(focussed_ws,
                                                          expected_peaks_dsp)
    ...
    def _fit_params(self, focused_ws, expected_peaks_d):
        ...
        fit_alg.setProperty('WorkspaceIndex',
                            0)  # There should be only one index anyway
    ...
    def _produce_outputs(self, difa, difc, zero, fitted_peaks):
        ...
        if '' != tblName:
            ...
            self.log().information(
                "Output parameters added into a table workspace: %s" % tblName)
