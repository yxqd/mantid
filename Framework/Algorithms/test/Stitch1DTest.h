...
private:
  template <typename T> class LinearSequence {
  private:
    const T m_start;
    const T m_step;
    T m_stepNumber;
  ...
  MatrixWorkspace_sptr createWorkspace(MantidVec &xData, MantidVec &yData,
  ...
  MatrixWorkspace_sptr createCosWaveWorkspace(const double startX,
                                              const double endX) {
...
public:
  ...
  ResultType do_stitch1D(MatrixWorkspace_sptr &lhs, MatrixWorkspace_sptr &rhs,
                         const double &startOverlap, const double &endOverlap,
                         const MantidVec &params, bool scaleRHS = true) {
    Stitch1D alg;
    alg.setChild(true);
    alg.setRethrows(true);
    alg.initialize();
  ...
  ResultType do_stitch1D(MatrixWorkspace_sptr &lhs, MatrixWorkspace_sptr &rhs,
                         const double &overlap, const MantidVec &params,
  ...
  void test_lhsworkspace_must_be_histogram() {
    ...
    TSM_ASSERT_THROWS("LHS WS must be histogram",
                      do_stitch1D(lhs_ws, rhs_ws, -1, 1, MantidVec(1, 0.2)),
  ...
  void test_rhsworkspace_must_be_histogram() {
    ...
    TSM_ASSERT_THROWS("RHS WS must be histogram",
                      do_stitch1D(lhs_ws, rhs_ws, -1, 1, MantidVec(1, 0.2)),
                      std::invalid_argument &);
  }
  ...
  void test_stitching_uses_suppiled_params() {
    ...
    MantidVec xValues = ret.get<0>()->readX(
        0); // Get the output workspace and look at the limits.
