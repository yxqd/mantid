...
public:
  ...
  void test_ZeroImaginary() {
    ...
    // uncomment line for debugging purposes only
    // The input real part was an exponential h*exp(-x^2/(2*s^2) with
    // h=1.0,s=1.0
...
private:
  /**
   * Check the maximum value stored in the workspace when the detailed condition
   * is applied
   * @param wsName name of the workspace
   * @param value compare to the maximum value stored in the Y-vector of the
   * workspace
   ...
   * Each spectra is a gaussian centered at the origin and with a different
   ...
   * Each workspace has nspectra, each spectra is a Gaussian centered at the
   * origin
  ...
  void createGroupWorkspace(const double *params, const std::string &gwsName) {
    ...
    // this->printWorkspace2D("/tmp/fqt.Re.dat",wsName); // uncomment line for
    ...
    sigma = params[3];
    this->createWorkspace2D(wsName, Heigth, sigma, nspectra);
    // this->printWorkspace2D("/tmp/fqt.Im.dat",wsName); // uncomment line for
    // debugging purposes only
    gws->add(wsName);
