...
namespace Mantid{
	namespace Algorithms{
		...
		/// Algorithm's name
		const std::string SetUncertainties::name() const {
			return "SetUncertainties";
		}

		/// Algorithm's version
		int SetUncertainties::version() const {
			return (1);
		}
		...
		void SetUncertainties::exec() {
			...
			std::string doubleComparisonAccuracy = getProperty("ComparisonAccuracy");
			...
			// count the decimal place specified
			int decimalPlaces = countDecimalPlaces(doubleComparisonAccuracy);
			...
			for (int64_t i = 0; i < int64_t(numHists); ++i) {
				PARALLEL_START_INTERUPT_REGION
					...
					// if custom error parameters, execute just those replacements
					if (customError) {
						auto &inputE = inputWorkspace->e(i);
						auto &outputE = outputWorkspace->mutableE(i);

						std::transform(inputE.begin(), inputE.end(), outputE.begin(), 
							[ifErrorEquals, newErrorValue](auto &inputValue)
						{
							// check if current error value is equal to parameter
							if (inputValue == ifErrorEquals) {
								// return value will be assigned to outputE
								return newErrorValue;
							}

							// return the same value if different from parameter
							return inputValue;
						...
						if (errorType.compare(ONE_IF_ZERO) == 0) {
							...
							outputWorkspace->mutableE(i) =
								std::vector<double>(inputWorkspace->e(i).size(), 0.);
						}
