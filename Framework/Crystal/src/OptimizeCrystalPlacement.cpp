...
void OptimizeCrystalPlacement::init() {
  ...
  declareProperty("MaxIndexingError", 0.15, "Use only peaks whose fractional "
  ...
  declareProperty("MaxHKLPeaks2Use", -1.0, "If less than 0 all peaks are used, "
                                           ...
                                           "are used(def=-1)");
  declareProperty("MaxSamplePositionChangeMeters", 0.001,
                  "Maximum Change in Sample position in meters(def=0.001)");
...
void OptimizeCrystalPlacement::exec() {
  ...
  if (perRun) {
    ...
    peaks->sort(criteria);
    const std::vector<Peak> &peaks_all = peaks->getPeaks();
    int run = 0;
    int count = -1;
    for (const auto &peak : peaks_all) {
      if (peak.getRunNumber() != run) {
        ...
        cloneWS->copyExperimentInfoFrom(peaks.get());
        runWS.push_back(cloneWS);
        runWS[count]->addPeak(peak);
        run = peak.getRunNumber();
        AnalysisDataService::Instance().addOrReplace(
            std::to_string(run) + peaks->getName(), runWS[count]);
      } else {
        runWS[count]->addPeak(peak);
      }
    }
  ...
  for (auto &i_run : runWS) {
    int nPeaksUsed = i_run->getNumberPeaks();
    //------------- Add initial parameter values to FuncArg -----------
    std::string FuncArg =
        "name=PeakHKLErrors,PeakWorkspaceName=" + i_run->getName();
    ...
    for (size_t i = 0; i < RunNumList.size(); i++) {
      ...
      if (k >= NOoptimizeRuns.size()) {
        ...
        oss1 << startConstraint << chiphiomega[0] - DegreeTol << "<chi"
             << runNum << "<" << chiphiomega[0] + DegreeTol;
    ...
    if (!(bool)getProperty("AdjustSampleOffsets")) {
      ...
      oss3 << "SampleXOffset=" << sampPos.X()
           << ",SampleYOffset=" << sampPos.Y()
           << ",SampleZOffset=" << sampPos.Z();
      Ties = oss3.str();
    }
    ...
    for (int prm = 0; prm < static_cast<int>(RRes->rowCount()); ++prm) {
      ...
      if (start != "chi" && start != "phi" && start != "ome" &&
          start != "Sam" && start != "Gon")
        continue;
    ...
    if (MapRunNum2GonMat.size() ==
        1) // Only one RunNumber in this PeaksWorkspace
    {
      Matrix<double> GonMatrix =
